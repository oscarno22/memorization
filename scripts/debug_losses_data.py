...
def compare_files(lr3_file, lr4_file):
    ...
    try:
        ...
        for key in set(lr3_data.get("meta", {}).keys()) | set(
            lr4_data.get("meta", {}).keys()
        ):
        ...
        if (
            "results" in lr3_data
            and "results" in lr4_data
            and "losses" in lr3_data["results"]
            and "losses" in lr4_data["results"]
        ):
            ...
            for loss_type in ["train", "test"]:
                ...
                if isinstance(lr3_losses, list) and isinstance(lr4_losses, list):
                    print(
                        f"  {loss_type}: lr3={len(lr3_losses)}, lr4={len(lr4_losses)}"
                    )
                    ...
                    print(
                        f"  {loss_type}: lr3={type(lr3_losses)}, lr4={type(lr4_losses)}"
                    )
